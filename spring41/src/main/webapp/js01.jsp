...
function find(y) {
	location.href= 'https://search.naver.com/search.naver?where=nexearch&sm=top_hty&fbm=0&ie=utf8&query=' + y
}
function know(z) {
...
<button onclick="go('naver')">네이버로이동</button>
<button onclick="find('자동차')">네이버에서 차동차 검색</button>
<button onclick="know('앤트로피')">네이버지식인에서 엔트로피 검색</button>
<button onclick="arr()">배열에 넣고, 프린트</button><hr><!-- F12번 콘솔창에 뜸 -->
...
<button onclick="data3()">pw를 입력해서 유효성 검증</button><br>
